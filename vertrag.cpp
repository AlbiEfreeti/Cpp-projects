...
{
    //Einsendeaufgabe 17.2: sichergehen,dass kein Datum in der Vergangenheit benutzt werden kann
    if (ui->dateEditAusleihe->date() < QDate::currentDate() || ui->dateEditRueckgabe->date() < QDate::currentDate() )
        {

            QMessageBox::warning(this, "Warnung", "Das ausgewählte Datum liegt in der Vergangenheit. Bitte wählen Sie ein gültiges Datum.");
            return;
        }
    ...
    QString abfrage = "UPDATE fahrzeuge SET ausgeliehen = 1 WHERE fNummer = " + ui->comboBoxFahrzeug->currentText();
    //und ausführen
    QSqlQueryModel *fahrzeugModell = new QSqlQueryModel;
    fahrzeugModell->setQuery(abfrage);
...
void Vertrag::preisBerechnen()
{
    ...
    QSqlQueryModel *fahrzeugModell = new QSqlQueryModel;
    QString preisAbfrage = "SELECT preisProTag FROM fahrzeuge WHERE fnummer = " + ui->comboBoxFahrzeug->currentText();
    fahrzeugModell->setQuery(preisAbfrage);
    preisProTag = fahrzeugModell->record(0).value(0).toFloat();
